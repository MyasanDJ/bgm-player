...
async function startAudio(){enforceAudible();if(!playlist.length){syncStatus();return}setupAudio();if(audioCtx&&audioCtx.state==="suspended")await audioCtx.resume();try{await audio.play();enforceAudible();syncStatus()}catch(e){setStatus("TAP AGAIN");console.log(e)}}

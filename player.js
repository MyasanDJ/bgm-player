...
const useNative=(isMobile || safeMode) && !sheetPopup;
const mainAudio=useNative?nativeAudio:audio;
...
const audioFiles=[{src:"bgm.mp3",type:"audio/mpeg"},{src:"bgm.m4a",type:"audio/mp4"},{src:"bgm.wav",type:"audio/wav"},{src:"bgm.wav",type:"audio/x-wav"}];
async function fileExists(url){try{const res=await fetch(url,{method:"HEAD",cache:"no-store"});return res.ok}catch(e){return false}}
async function loadFirstExistingAudio(){
  for(const file of audioFiles){
    if(await fileExists(file.src)){
      ...
      mainAudio.load();
      if(shouldAutoplay&&!useNative) setTimeout(()=>startAudio(),250);
      return;
    }
  }
}
...
const coverFiles=["cover.png","cover.jpg","cover.jpeg"];let coverIndex=0;
function loadCover(){coverImg.onerror=()=>{coverIndex++;if(coverIndex<coverFiles.length)coverImg.src=coverFiles[coverIndex];else coverImg.style.display="none"};coverImg.onload=()=>{coverImg.style.display="block"};coverImg.src=coverFiles[coverIndex]}
loadCover();
...
function save(){
  ...
  localStorage.setItem("bgmState",JSON.stringify({volume:mainAudio.volume,muted:mainAudio.muted,mini:player.classList.contains("mini"),hidden:player.classList.contains("hidden"),left:player.style.left,top:player.style.top}));
}
function load(){
  if(sheetPopup)return;
  try{
    const s=JSON.parse(localStorage.getItem("bgmState")||"{}");
    if(typeof s.volume==="number"&&!useNative){mainAudio.volume=s.volume;volume.value=Math.round(s.volume*100);volText.textContent=volume.value+"%"}
    if(s.muted&&!useNative){mainAudio.muted=true;mute.textContent="🔇"}
...
mainAudio.addEventListener("play",async()=>{
  ...
  if(!useNative){cancelAnimationFrame(raf);draw()}
});
mainAudio.addEventListener("pause",()=>{
  play.textContent="▶";
  player.classList.remove("playing");
  if(!useNative){cancelAnimationFrame(raf);drawIdle()}
});
...
// Mobile/safe: background移行前に明示停止して、復帰後のWebAudio破損を防ぐ

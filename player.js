...
let wakeLock=null;
async function requestWakeLock(){try{if("wakeLock" in navigator)wakeLock=await navigator.wakeLock.request("screen")}catch(e){console.log("Wake Lock unavailable",e)}}
function updateMediaSession(){
  if(!("mediaSession" in navigator))return;
  const t=playlist[currentIndex]||{};
  const art=t.cover?[{src:t.cover,sizes:"96x96",type:"image/png"},{src:t.cover,sizes:"128x128",type:"image/png"},{src:t.cover,sizes:"192x192",type:"image/png"},{src:t.cover,sizes:"256x256",type:"image/png"},{src:t.cover,sizes:"512x512",type:"image/png"}]:[];
  navigator.mediaSession.metadata=new MediaMetadata({title:t.title||"BGM",artist:"MyasanDJ",album:"Spreadsheet BGM",artwork:art});
  navigator.mediaSession.playbackState=audio.paused?"paused":"playing";
}
function setupMediaSessionHandlers(){
  if(!("mediaSession" in navigator))return;
  navigator.mediaSession.setActionHandler("play",async()=>{await startAudio()});
  navigator.mediaSession.setActionHandler("pause",()=>{audio.pause();syncStatus();updateMediaSession()});
  ...
  navigator.mediaSession.setActionHandler("seekforward",d=>{audio.currentTime=Math.min(audio.duration||audio.currentTime,audio.currentTime+(d.seekOffset||10))});
...
function loadTrack(i,autoplay=true){
  ...
  updateText();loadCover(t);updateMediaSession();seek.value=0;current.textContent="0:00";duration.textContent="0:00";
...
audio.addEventListener("play",()=>{syncStatus();updateMediaSession();cancelAnimationFrame(raf);draw()});
audio.addEventListener("pause",()=>{syncStatus();updateMediaSession();cancelAnimationFrame(raf);drawIdle()});
...
audio.onloadedmetadata=()=>{seek.max=audio.duration||0;duration.textContent=fmt(audio.duration);if(audio.paused)setStatus("READY");updateMediaSession()};
...
// v6.0: バックグラウンド移行時に自動停止しない。復帰時だけ表示を同期。
...
drawIdle();setupMediaSessionHandlers();loadState();updateLoopButton();updateShuffleButton();loadTrack(0,false);if(shouldAutoplay&&!isMobile)setTimeout(()=>startAudio(),400);
